...
def add_host_details(hosts):
    ...
    if (online):
        ...
        data_hostnames = read_json_from_file(f'{base_path}/data_sysname_{instance_string}.json')
    ...
    # Get model name.
    if (online):
        query = 'entPhysicalModelName{entPhysicalModuleName!="ModuleSlot"}'
        data_model = execute_snmp_query(query)
    else:
        data_model = read_json_from_file(f'{base_path}/data_model_{instance_string}.json')

    for host in hosts:
        for metric in data_model['data']['result']:
            if metric['metric']['instance'] in host['IPv4']:
                if 'entPhysicalModelName' in metric['metric']:
                    host['device_model'] = metric['metric']['entPhysicalModelName']

    # Get serial number.
    if (online):
        query = 'entPhysicalSerialNum{entPhysicalSerialNum!=""}'
        data_model = execute_snmp_query(query)
    else:
        data_model = read_json_from_file(f'{base_path}/data_serialno_{instance_string}.json')

    for host in hosts:
        for metric in data_model['data']['result']:
            if metric['metric']['instance'] in host['IPv4']:
                if 'entPhysicalSerialNum' in metric['metric']:
                    host['serial_number'] = metric['metric']['entPhysicalSerialNum']
